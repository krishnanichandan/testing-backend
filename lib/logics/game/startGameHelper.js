...
function processStartGame(data) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let result = yield pipeline.run(data).catch((e) => {
            var _a;
            console.log("EXCEPTION HERE in start game", (_a = e.processedData) === null || _a === void 0 ? void 0 : _a.errorData);
            catchedError = e;
...
exports.processStartGame = processStartGame;
...
//will check it again
let checkIfGameAlreadyStart = new j_stillery_1.Task((input, resolve, reject) => {
    resolve(input);
});
let getTableDataFromDb = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
    //Fetching table:
    let table = yield LockerHelper.getTable(input.tableId, "StartGameProcess").catch(e => { });
    if (!table) {
    ...
    if (![`${types_1.GameState.Idle}`, `${types_1.GameState.Betting}`].includes(`${table.currentInfo.state}`)) {
...
let nestedCheckGameIdleState = new j_stillery_1.Task((input, resolve, reject) => {
    const gameState = input.processedData.table.currentInfo.state;
    if (gameState === types_1.GameState.Idle) {
        //initializing Data
        let initData = new j_stillery_1.Task((input, resolve, reject) => {
        ...
        let resetPlayerState = new j_stillery_1.Task((input, resolve, reject) => {
            // const players = [];
        ...
        let scheduleInactivityJobForPlayers = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
            // Extract the game table from the input payload
            let table = input.processedData.table;
            table.currentInfo.players.forEach((player) => {
                SchedulerHelper_1.SchedulerHelper.Instance.removeInactivePlayer(input.tableId, player.playerId);
            });
        ...
        let changePlayerStateToBetting = new j_stillery_1.Task((input, resolve, reject) => {
            const players = [];
            input.processedData.table.currentInfo.players.forEach((player) => {
                if (player.chips >= input.processedData.table.info.minBuyIn) {
                    if (player.state === types_1.PlayerState.Waiting || player.state === types_1.PlayerState.Playing || player.state === types_1.PlayerState.Disconnected) {
                        player.state = types_1.PlayerState.Betting;
                        player.active = true;
            ...
            if (players.length) {
                players.forEach(player => {
                    SchedulerHelper_1.SchedulerHelper.Instance.removePlayerIfSessionExceed(input.tableId, player.playerId);
                });
                (0, broadcaster_1.dispatchPlayerPlaySessionBroadCast)(input.room, { players: players });
            }
            resolve(input);
        });
        let askForContinueBetPhaseAndGameSessionToPlayers = new j_stillery_1.Task((input, resolve, reject) => {
            if (input.processedData.table.currentInfo.showBetPhaseContinuePopUpRemaining) {
                ...
                if (players.length) {
                    players.forEach(player => {
                        SchedulerHelper_1.SchedulerHelper.Instance.removePlayerIfBetPhasePopNotResponsd(input.tableId, player.playerId);
                    });
                    console.log("PlayerBetContinuePhaseBroadcast->", JSON.stringify(players));
                    (0, broadcaster_1.dispatchPlayerBetContinuePhaseBroadcast)(input.room, { players: players });
        ...
        let isGameGoingToStart = new j_stillery_1.Task((input, resolve, reject) => {
            let table = input.processedData.table;
            let bettingPlayers = (0, TableHelper_1.getPlayersByState)(table, types_1.PlayerState.Betting); //table.currentInfo.players.filter(player => (player.state === PlayerState.Playing));
        ...
        let sortPlayerIndexes = new j_stillery_1.Task((input, resolve, reject) => {
            let table = input.processedData.table;
        ...
        let removeTournamentPlayers = new j_stillery_1.Task((input, resolve, reject) => {
            resolve(input);
        });
        // If enough players to start the Betting Phase of game
        let isEnoughBettingPlayers = new j_stillery_1.Task((input, resolve, reject) => {
            let table = input.processedData.table;
            let totalActivePlayersResponse = (0, TableHelper_1.totalActiveBettingPlayers)(input.processedData);
        ...
        let inserRoundId = new j_stillery_1.Task((input, resolve, reject) => {
            let table = input.processedData.table;
        ...
        let gamePlayersData = new j_stillery_1.Task((input, resolve, reject) => {
            input.processedData.data.dataPlayers = JSON.parse(JSON.stringify((0, underscore_1.map)(input.processedData.idleStateData.players, function (player) { return (0, underscore_1.pick)(player, 'playerId', 'playerName', 'chips', 'state', "seatIndex", "active"); })));
            resolve(input);
        });
        let checkBettingPhaseStart = new j_stillery_1.Task((input, resolve, reject) => {
        ...
        let changeGameStateAndTableAttributes = new j_stillery_1.Task((input, resolve, reject) => {
            if (input.processedData.data.bettingPhaseStart) {
                input.processedData.table.currentInfo.state = types_1.GameState.Betting;
        ...
        let checkContinueBettingPhase = new j_stillery_1.Task((input, resolve, reject) => {
        ...
        let fireGamePlayersBroadcast = new j_stillery_1.Task((input, resolve, reject) => {
            ...
            if (input.processedData.data.preGameState !== types_1.GameState.Idle) {
            ...
            if (input.processedData.data.bettingPhaseStart) { //or check data.startGAme
                ...
                setTimeout(function () {
                    ...
                    (0, broadcaster_1.dispatchTablePlayersBroadcast)(input.room, { tableId: input.processedData.table.id, players: input.processedData.data.dataPlayers, spectator: input.processedData.data.spectatorPlayer });
                }, (300));
        ...
        function resetTableOnNoBettingStart(table) {
            ...
            // Resetting players
            table.currentInfo.players.forEach((player) => {
                if ([types_1.PlayerState.Betting || types_1.PlayerState.Disconnected || types_1.PlayerState.Playing || types_1.PlayerState.Waiting].includes(player.state)) {
                    player.state = types_1.PlayerState.Waiting;
                    player.active = false;
        ...
        pipeline.run(input).then((result) => __awaiter(void 0, void 0, void 0, function* () {
            if (input.processedData.data.bettingPhaseStart) {
                //fire timer broadcast
                //start timer For all players
                input.processedData.table.currentInfo.maxBettingCountOnTable -= 1;
                let table = yield (0, Queries_1.replaceTable)(input.processedData.table);
                console.log("after replacing table ");
                // start Betting Timer
                let bettingTimer = input.room.clock.setTimeout(function (params) {
                    ...
                    (0, broadcaster_1.dispatchStartBettingBroadCasts)(input.room, { tableId: input.tableId, bettingPlayers: input.processedData.idleStateData.bettingPlayers });
                    (0, timerHelper_1.startTurnTimer)({ room: input.room, table: input.processedData.table, bettingTime: true });
                }, 800, input);
                reject(input);
                return;
            }
            ...
            input.processedData.table.currentInfo.state = types_1.GameState.Idle;
            input.processedData.table.currentInfo.stateInternal = types_1.GameState.Idle;
            ...
            input.processedData.data.dataPlayers = JSON.parse(JSON.stringify((0, underscore_1.map)(input.processedData.table.currentInfo.players, function (player) { return (0, underscore_1.pick)(player, 'playerId', 'playerName', 'chips', 'state', "seatIndex", "active"); })));
            setTimeout(function () {
...
let nestedChekGameBettingState = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
    let table = input.processedData.table;
    if (table.currentInfo.state === types_1.GameState.Betting) {
        ...
        let checkPlayerBettingState = new j_stillery_1.Task((input, resolve, reject) => {
            let bettingPlayers = table.currentInfo.players.filter((player) => player.state === types_1.PlayerState.Betting && player.active === true);
        ...
        let checkMinReadyPlayer = new j_stillery_1.Task((input, resolve, reject) => {
            let readyPlayers = table.currentInfo.players.filter((player) => (player.state === types_1.PlayerState.Ready || (player.state === types_1.PlayerState.Disconnected && player.initialBet > 0)) && player.active === true);
        ...
        let resetPlayerState = new j_stillery_1.Task((input, resolve, reject) => {
            for (let i = 0; i < input.processedData.table.currentInfo.players.length; i++) {
                let player = input.processedData.table.currentInfo.players[i];
                if (player.state === types_1.PlayerState.Disconnected && player.initialBet >= table.info.minBuyIn) {
                    //player has placed some bet in betting phase but got Disconnected
                    player.state = types_1.PlayerState.Playing;
                    player.active = true;
                    player.isWaitingPlayer = false;
                    // SchedulerHelper.Instance.removeSitoutPlayer(input.tableId, player.playerId);
                }
                if (player.initialBet >= table.info.minBuyIn && player.state === types_1.PlayerState.Ready) {
                    player.state = types_1.PlayerState.Playing;
                    player.active = true;
                    player.isWaitingPlayer = false;
                }
                if (player.initialBet >= 0 && player.state === types_1.PlayerState.Waiting) {
                    player.state = types_1.PlayerState.Waiting;
                    player.active = true;
                    player.isWaitingPlayer = false;
        ...
        let isGameGoingToStart = new j_stillery_1.Task((input, resolve, reject) => {
            let table = input.processedData.table;
            let waitingPlayers = (0, TableHelper_1.getPlayersByState)(table, types_1.PlayerState.Waiting); //table.currentInfo.players.filter(player => (player.state === PlayerState.Waiting));
            let playingPlayers = (0, TableHelper_1.getPlayersByState)(table, types_1.PlayerState.Playing); //table.currentInfo.players.filter(player => (player.state === PlayerState.Playing));
            // If less than 1 players then do not start game
            // Do not perform consider player logic. Return function from here
        ...
        let shuffleDeck = new j_stillery_1.Task((input, resolve, reject) => {
            let table = input.processedData.table;
            let numOfDecks = parseInt(JSON.parse(JSON.stringify(table.info.noOfDeck)));
            let deck = [];
            while (numOfDecks >= 1) {
                deck = deck.concat(new Deck_1.Deck().getCards());
                numOfDecks--;
            }
        ...
        let distributeCardsTask = new j_stillery_1.Task((input, resolve, reject) => {
            let response = (0, CardDistributer_1.distributeCards)(input.processedData.table);
        ...
        let sortPlayerIndexes = new j_stillery_1.Task((input, resolve, reject) => {
            ...
            let inactivePlayer = [];
            table.currentInfo.players.forEach((player) => {
                if (player.state !== types_1.PlayerState.Playing) {
        ...
        let checkInsurancePlace = new j_stillery_1.Task((input, resolve, reject) => {
        ...
        let checkPlayerAvailableAction = new j_stillery_1.Task((input, resolve, reject) => {
        ...
        let changeGameState = new j_stillery_1.Task((input, resolve, reject) => {
            if (input.processedData.data.startGame) {
                input.processedData.table.currentInfo.state = types_1.GameState.Running;
            }
            resolve(input);
        });
        //to do 
        ...
        let gamePlayersData = new j_stillery_1.Task((input, resolve, reject) => {
            input.processedData.data.dataPlayers = JSON.parse(JSON.stringify((0, underscore_1.map)(input.processedData.table.currentInfo.players, function (player) { return (0, underscore_1.pick)(player, 'playerId', 'playerName', 'chips', 'state', "seatIndex", "active", "handInfo", "initialBet"); })));
            resolve(input);
        });
        let arr = [
...
let fireGamePlayersBroadcast = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
    // logger.info("fireGamePlayersBroadcast started", input)
    // console.log(1)
    // console.log("fireGamePlayersBroadcast");
    if (!input.eventName) {
    ...
    if (input.processedData.data.preGameState !== types_1.GameState.Betting) {
...
let setOnBreakAndStartReserveTimer = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    input.processedData.table.currentInfo.players.forEach((player) => {
        if (player.state === types_1.PlayerState.OutOfMoney) {
            ...
            player.state = types_1.PlayerState.OutOfMoney;
            // SchedulerHelper.Instance.removeSitoutPlayer(input.tableId, player.playerId);
            (0, broadcaster_1.dispatchPlayerStateBroadcast)(input.room, { tableId: input.tableId, playerId: player.playerId, playerState: player.state });
        }
    });
    if (isAnyBankrupt) {
        let table = yield (0, Queries_1.replaceTable)(input.processedData.table).catch(e => { });
...
let checkGameStart = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
...
let fireCardDistributeBroadcast = new j_stillery_1.Task((input, resolve, reject) => {
    // console.log(4)
    let ts = JSON.parse(JSON.stringify((0, underscore_1.map)(input.processedData.data.players, function (player) { return (0, underscore_1.pick)(player, 'playerId', 'playerName', 'chips', 'state', "seatIndex", "handInfo"); })));
    ...
    setTimeout(function () {
        ...
        (0, broadcaster_1.dispatchDistributeCardsBroadcast)(input.room, { tableId: input.processedData.table.id, players: ts, numCards: input.processedData.data.distributeCardsResponse.numCards, dealer: dealer });
    }, (3000));
    resolve(input);
});
//fire insurance Broadcast
//to do
let fireStartGameBroadcast = new j_stillery_1.Task((input, resolve, reject) => {
    // console.log(6)
    setTimeout(function () {
        (0, broadcaster_1.dispatchStartGameBroadcast)(input.room, input.processedData.table);
    }, (600));
...
let fireInsuranceBroadCastAndTimer = new j_stillery_1.Task((input, resolve, reject) => {
    if (input.processedData.data.canInsurancePlace) {
        let ts = JSON.parse(JSON.stringify((0, underscore_1.map)(input.processedData.data.players, function (player) { return (0, underscore_1.pick)(player, 'playerId', 'playerName', 'chips', 'state', "seatIndex", "handInfo"); })));
        const insurancePlayers = [];
        ts.forEach((player) => {
        ...
        if (insurancePlayers.length) {
            ...
            input.processedData.table.currentInfo.state = types_1.GameState.Starting;
            let cardDistributeDelay = 1.8;
            let cardAnimDelay = 0;
            let totalPlayer = insurancePlayers.length + 1;
            let numCards = 2;
            let insuranceTimer = input.room.clock.setTimeout(function (params) {
                console.log("insurance Broadcast Fired");
                (0, broadcaster_1.dispatchInsuranceBroadCasts)(input.room, { tableId: input.tableId, players: insurancePlayers });
                (0, timerHelper_1.startTurnTimer)({ room: input.room, table: JSON.parse(JSON.stringify(input.processedData.table)), insuranceTime: true });
...
let fireOnTurnBroadCast = new j_stillery_1.Task((input, resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
